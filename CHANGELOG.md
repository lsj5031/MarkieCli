...
- Add a CI workflow (`.github/workflows/ci.yml`) that runs `cargo test` + `cargo clippy` and then regenerates the demos in a pinned font container to verify the committed assets are still in sync with the renderer; the check script (`scripts/check-demos.sh`) compares SVG content structurally (ignoring platform font-metric geometry) so it works across macOS/Linux.
...
35 commits since v0.4.0 across 32 files (+3,154 / −536 lines). This release focuses on rich inline HTML rendering (superscripts, subscripts, colors, highlights), fixes for content loss (HTML blocks) and layout overflow (tables), hardened remote image fetching, strict Mermaid diagram-type errors, plus the two security fixes landed earlier in the cycle.
